          ...
          {state.finalScores.map(fs => (
            <div
              ...
              className={`card-container flex items-center gap-4 py-4
                ${fs.playerId === state.playerId ? 'border-green-600/50' : ''}
              `}
            >
              ...
              <div className="flex-1">
                <p className={`font-semibold ${fs.playerId === state.playerId ? 'text-green-400' : ''}`}>
                  {fs.playerName}
                  {fs.playerId === state.playerId && <span className="text-xs text-gray-500 ml-2">(Du)</span>}
                </p>
              </div>
              <div className="font-mono text-lg text-green-400">{fs.score}</div>
            </div>
          ))}
        </div>
